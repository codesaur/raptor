...
{    
    public function index()
    {
        ...
        if ($table && $table != 'accounts') {
            $modal = velociraptor_html . "/account/$table-index-modal.html";
            ...
            $response = $this->indopost("/record/retrieve?table=$table&model="
                    . \urlencode('Indoraptor\\Models\\' . ($table == 'forgot' ? 'Forgot' : 'Accounts')),
            ...
            return (new TwigTemplate(
                    $modal,
                    array(
                        'user' => single::user(), 'login' => new LoginController(),
        ...
        $view->addDelete(array('logger' => 'account', 'model' => 'Indoraptor\\Models\\Accounts'), 'table');
        ...
        $organizations_result = $this->indopost('/record/retrieve?model=' . \urlencode('Indoraptor\\Models\\Organizations'));
        $vars = array(
            'accounts' => $accounts,
        ...
        $view->renderTwig(velociraptor_html . '/account/account-index.html', $vars);
    }
    ...
    public function crud(string $action, $id)
    {
        try {
            ...
            if (isset($id)) {
                $response = $this->indopost('/record/retrieve?model='
                        . \urlencode('Indoraptor\\Models\\Accounts'), array('id' => $id));
                ...
                $photo = (new ImageController("/account/$id"))->setTable('accounts')->single($id);
            }
            
            $column  = (new AccountDescribe())->getTwigColumns($response['data']['record'] ?? array());
            $column += (new ImageController())->getAsTwigColumn('image', $photo ?? array());
            ...
            if ($permission == 'organization') {
                ...
                $delete = array(    
                    'table' => 'accounts', 'logger' => 'account',
                    'postBack' => $this->getMe() . '::onDeleteImage');
                $view->addDelete(
                        $delete, '#tab-picture',
                        single::text('delete-image-ask'), null, 'strip_file');
                
                $view->renderTwig(velociraptor_html . "/account/account-$action.html", $vars);
            }
    ...
    public function submit(string $action)
    {
        try {
            ...
            if ($action == 'update') {
                ...
                if ($password_set) {
                    $response = $this->indopost('/record/retrieve?model='
                            . \urlencode('Indoraptor\\Models\\Accounts'), array('id' => $auto_increment));
            ...
            $response = $this->$method('/record?model='
                    . \urlencode('Indoraptor\\Models\\Accounts'), array('record' => $record));
    ...
    public function organizationSet($id)
    {
        if (single::request()->getMethod() == 'GET') {
            ...
            (new TwigTemplate(velociraptor_html . '/account/account-organization-set.html', $vars))->render();
            ...
            foreach ($org_user_result['data'] ?? array() as $row) {
                if (isset($organizations[(int) $row['organization_id']])) {
                    ...
                    $response_org_delete = $this->indodelete('/record?model='
                            . \urlencode('Indoraptor\\Models\\OrganizationUsers'), array('id' => $row['id']));
            ...
            foreach (\array_keys($organizations) as $value) {
                $response_org_set = $this->indopost('/record?model='
                        . \urlencode('Indoraptor\\Models\\OrganizationUsers'),
    ...
    public static function onDeleteImage(array $record)
    {
        if ($record['record'] == single::user()->account('id')) {
            single::session()->release("dashboard/account/{$record['record']}/picture");
        }
    }
    ...
    public function orgIndex(Dashboard $view)
    {
        ...
        if (single::user()->can($alias . '_rbac_index')) {
            $rbac_link = single::link('crud', array('action' => 'index')) . 
                    '?logger=rbac&controller=' . \urlencode('Velociraptor\\Controllers\\RBACController') .
        ...
        $view->renderTwig(velociraptor_html . '/account/org-account-index.html', $vars);
    ...
    public function orgCRUDInsert()
    {
        ...
        (new TwigTemplate(velociraptor_html . "/account/org-account-insert-modal.html", $values))->render();
    }
    ...
    public function kick()
    {
        try {
            ...
            $response_org_delete = $this->indodelete('/record?model='
                    . \urlencode('Indoraptor\\Models\\OrganizationUsers'), array('id' => $org_user_result['data'][0]['id']));
