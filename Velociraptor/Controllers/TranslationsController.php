...
class TranslationsController extends RaptorController
{
                ...
                foreach ($tables as $table) {
                    ...
                    if (single::user()->can('system_translation_initial')) {
                        $cards[$table]->addButton(single::text('get-initial-code'),
                                ...
                                "$query&model=" . \urlencode('Indoraptor\\Models\\Translation') .
                                '&describe=' . \urlencode('Indoraptor\\Describes\\TranslationDescribe'),
        ...
        try {
            ...
            if (isset($id)) {
                $response = $this->indopost('/record/retrieve?model='
                        . \urlencode('Indoraptor\\Models\\Translation') . "&table=$table", array('id' => $id));
            ...
            (new TwigTemplate(
                velociraptor_html . '/translation/crud-action-modal.html',
                array(
                    'crud' => $crud,
                    'action' => $action,
                    'column' => $column,
    ...
    public function submit(string $action)
    {
        try {
            ...
            $response = $this->$method(
                    "/record?table=$table&model=" . \urlencode('Indoraptor\\Models\\Translation'),
    ...
    {
        $rows = array();
        $response = $this->indopost("/record/retrieve?table=$table&model=" . \urlencode('Indoraptor\\Models\\Translation'));
