...
use Indoraptor\Describes\OrganizationDescribe;
...
class OrganizationController extends RaptorController
{
    public function index()
    {
        ...
        $card->addContent(new TwigTemplate(velociraptor_html . '/organization/index-table.html'));
        
        $view->addDelete(array('logger' => 'organization', 'model' => 'Indoraptor\\Models\\Organizations'));
        ...
        try {
            ...
            if (isset($id)) {
                $response = $this->indopost('/record/retrieve?model='
                        . \urlencode('Indoraptor\\Models\\Organizations'), array('id' => $id));
            ...
            if (single::user()->can('system_org_index')) {
                ...
                $orgs_response = $this->indopost('/record/retrieve?model='
                        . \urlencode('Indoraptor\\Models\\Organizations'),
            ...
            (new TwigTemplate(velociraptor_html . '/organization/crud-action-modal.html', $vars))->render();
    ...
    public function submit(string $action)
    {
        ...
        $response = $this->$method('/record?model='
                . \urlencode('Indoraptor\\Models\\Organizations'), array('record' => $record));
    ...
    public function datatable()
    {
        ...
        if (single::user()->can('system_org_index')
            ...
            $rows = array();
            $rbac_link = single::link('crud', array('action' => 'index')) . '?logger=rbac&controller=' . \urlencode('Velociraptor\\Controllers\\RBACController');
