...
class SettingsController extends RaptorController
{
    ...
    public function settings()
    {
        ...
        $column = (new SettingDescribe())->getTwigColumns($record);
        ...
        $view->renderTwig(
                velociraptor_html . '/settings/settings.html',
    ...
    public function socials()
    {
        ...
        $column = (new SocialDescribe())->getTwigColumns($record);
        if (empty($record)) {
            $column['alias']['value'] = single::user()->organization('alias');
        }

        (new TwigTemplate(
                velociraptor_html . '/settings/socials-modal.html',
                array(
                    'column' => $column,
                    'action' => single::link('crud-submit', array('action' => empty($record) ? 'insert' : 'update'))
    ...
    public function mailer()
    {
        ...
        (new TwigTemplate(
                velociraptor_html . '/settings/mailer-modal.html',
                array(
    ...
    public function submit(string $action, $table)
    {
        ...
        if ($table == 'mailer') {
            ...
            $model = \urlencode('Indoraptor\\Models\\Mailer');
        } elseif ($table == 'socials') {
            if ( ! single::user()->can(single::user()->organization('alias') . '_website_socials')) {
                return false;
            }
            
            $describe = new SocialDescribe();
            $record = $describe->getPostValues();
            $model = \urlencode('Indoraptor\\Models\\Socials');
        } elseif ($table == 'settings') {
            if ( ! single::user()->can(single::user()->organization('alias') . '_website_settings')) {
                return false;
            }

            $describe = new SettingDescribe();
            $record = $describe->getPrimary()->getPostValues();
            $content = $describe->getContent()->getPostValues(single::language()->codes());
            $model = \urlencode('Indoraptor\\Models\\Settings');
        } else {
            return false;
        }
